...
Dans cette partie nous allons tenter de determiner si il existe une relation entre la géographie (Académies) et la répartition des élèves entre les différentes filliaires générale du lycée. Nous ne savons pas si la répartition répond a une forme de quotat de la part du ministaire.

Nous avont choisir d'utiliser la méthode du khi-deux car cela répond particulièrement bien aux besoins de cette analyse de dépendance entre deux variables qualitative (Des filliaires et des Académies). De plus nous avons choisis de montrer les différentes étapes du khi-deux, dans un soucis de clarté mais également pour avoir une vision de l'impact de chaque combinaison académie/filiaire sur le résultat. Ce derniers point nous permetra de présenter une analyse plus fine de la dépendance entre les deux variables.

Notre hypothèse de départ est que la répartition des éléves entre les filiaires générale des lycée, d'une académie à l'autre, ne devrait pas changer.

Nous commencons donc par aggreger les données de la base de données pour créer un tableaux qui servira de point de départ à l'application de la méthode. Nous avons aggregé les données présentes dans la base de données en sommant les effectifs par académie. Nous avont également restreint notre etude aux colonnes qui nous interessent, les effectifs présent dans les filliaires générales.
...
A partir de cette table nous allons calculer une table des valeurs théoriques que nous devrion trouver dans le tableau en fonction des effectifs totaux pour chaque filliaire et académie. (Voir Annexe1)
